#!/usr/bin/env node
/**
 * Docker Hub deployment script for Ghostarr
 *
 * Features:
...
function getDockerConfig() {
  const pkg = JSON.parse(fs.readFileSync('package.json', 'utf8'));
  return {
    image: pkg.config?.dockerImage || 'sharkhunterr/ghostarr',
...
function main() {
  console.log('🐳 Ghostarr Docker Deployment Script\n');
  ...
  if (options.multiPlatform) {
    ...
    exec(
      'docker buildx create --use --name ghostarr-builder 2>/dev/null || docker buildx use ghostarr-builder',
